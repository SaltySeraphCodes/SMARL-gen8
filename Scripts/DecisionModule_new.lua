...
function DecisionModule.getFinalTargetBias(self, perceptionData)
    ...
    elseif currentMode == "AvoidWallRight" then
        targetBias = -WALL_STEERING_BIAS
    elseif self.currentMode == "OvertakeDynamic" or self.currentMode == "AvoidCollision" or (perceptionData.Opponents.count > 0) then
        -- Receive both Bias AND DebugData
        local bias, debugData = self:calculateContextBias(perceptionData)
        -- Store debug data in the module so DriverGen8 can find it
