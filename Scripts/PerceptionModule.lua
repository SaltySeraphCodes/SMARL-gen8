...
function PerceptionModule:getPointInDistance(baseNode, start_t, distance, chain)
    ...
    while remainingDistance > 0 and nodeDitsTimeout < 100 do
        ...
        if effectiveLength >= remainingDistance then
            local target_t = (nodeDitsTimeout == 0) and (start_t + (remainingDistance / segmentLength)) or
    ...
    if nav.closestPointData then
        ...
        while scanDist < MAX_SCAN_DIST and scanNode do
            ...
            if r < TURN_THRESH then
                ...
                local cross = v1:cross(v2).z
                if cross < -0.001 then
                    nav.nextCornerDir = 1  -- Right
                elseif cross > 0.001 then
                    nav.nextCornerDir = -1 -- Left
                end
        ...
        if driver.id ~= self.Driver.id and driver.perceptionData then
            ...
            if dist < 100.0 and dist > 0.01 then
                ...
                table.insert(list, {
                    ...
                    opponentBias = driver.perceptionData.Navigation and
                        driver.perceptionData.Navigation.trackPositionBias or 0.0
                })
            end
        end
    end
...
function PerceptionModule.performRaycasts(self)
    ...
    local speed = (self.perceptionData.Telemetry and self.perceptionData.Telemetry.speed) or 0
    local scanLen = math.max(15.0, speed * 1.1) -- Look ~1.1 seconds ahead
    local centerLen = math.max(30.0, speed * 1.5)

    local lDir = (fwd * 0.96) - (right * 0.25)
    local lEnd = startPos + (lDir * scanLen)
    local lValid, lRes = sm.physics.raycast(startPos, lEnd)
    if lValid and lRes.type ~= "Body" then
        results.left = (lRes.pointWorld - startPos):length()
    end

    -- RAY 2: RIGHT ANGLE (15 deg)
    local rDir = (fwd * 0.96) + (right * 0.25)
    local rEnd = startPos + (rDir * scanLen)
    ...
    local cEnd = startPos + (fwd * centerLen)
