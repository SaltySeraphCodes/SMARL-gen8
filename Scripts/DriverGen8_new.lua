...
function DriverGen8.server_onFixedUpdate(self, dt)
    ...
    if self.Decision and self.perceptionData then
        self.Decision.pitState = self.pitState
        decisionData = self.Decision:server_onFixedUpdate(self.perceptionData, dt)
        self.decisionData = decisionData
        
        -- [[ VISUALIZATION NETWORKING ]]
        -- Check if DecisionModule generated debug lines
        if self.Decision.latestDebugData then
            -- Throttle: Send only every 3 ticks (13 FPS update rate is enough for debug)
