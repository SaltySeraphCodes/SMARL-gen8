...
function DecisionModule.getFinalTargetBias(self, perceptionData)
    ...
    if wallCheck or isOpponentDanger or VISUALIZE_RAYS then
        ...
        -- [[ FIX: STRONG SAFETY ]]
        -- If danger is present, override the racing line completely (1.0).
        -- Otherwise blend gently for soft obstacles.
        local isWallDanger = (wall and (wall.isForwardLeftCritical or wall.isForwardRightCritical))

        if math.abs(safeBias - idealBias) > 0.1 then
            local blend = 0.5
            if isWallDanger or isOpponentDanger then blend = 1.0 end
...
function DecisionModule.determineStrategy(self, perceptionData, dt)
    ...
    if self.Driver.formation then
        self.currentMode = "Formation"; return
    elseif self.Driver.caution then
        self.currentMode = "Caution"; return
    elseif self.pitState > 0 then
        self.currentMode = "Pit"; return
    end
