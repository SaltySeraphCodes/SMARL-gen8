...
function GuidanceModule:calculateStanley(driver, targetPoint, pathHeading)
    ...
    local cte = vecToTarget:dot(tm.rotations.right) *
    -1.0                                                   -- [FIX] Inverted Vector, and CTE should be Positive for Right Target (Steer into it)
...
function GuidanceModule.server_onFixedUpdate(self, dt, decisionData)
    ...
    local centerPoint = self.Driver.Decision:getFutureCenterPoint(baseNode, t, lookaheadDist,
    ...
    -- [[ FIX: DYNAMIC LANE OFFSETS ]]
    -- 1. Base Offset (Formation)
    local formationSide = self.Driver.formationSide or 0
    local laneOffset = formationSide * 3.0 -- 3.0m offset

    -- 2. Blend Out in Corners
    -- If we are aiming for an apex (targetBias != 0) OR curvature is high
    local corneringIntensity = math.abs(targetBias) * 2.0
    if nav.curvature then
        corneringIntensity = math.max(corneringIntensity, math.abs(nav.curvature) * 20.0)
    end

    -- Smooth blend: If intensity > 1.0, offset is 0.
    local blend = math.max(0.0, 1.0 - corneringIntensity)
    laneOffset = laneOffset * blend

    -- 3. Combine
    -- Center - (Perp * (RacingLine + LaneOffset))
    local totalOffset = (targetBias * (trackWidth * 0.5)) + laneOffset
    local targetPoint = centerPoint - (perp * totalOffset)
